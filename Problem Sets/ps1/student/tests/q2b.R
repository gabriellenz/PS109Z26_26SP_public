      ...
      code = {
        ...
        try({
          ...
          question.correct <- inherits(california_plot, "gg") &&
            ...
            isTRUE(all.equal(correct.plot$mapping[["x"]], california_plot$mapping[["x"]])) &&
            isTRUE(all.equal(correct.plot$mapping[["y"]], california_plot$mapping[["y"]])) &&
            isTRUE(all.equal(correct.plot$mapping[["colour"]], california_plot$mapping[["colour"]])) &&
            geom.ok && facet.ok && log.ok
        }, silent = TRUE)
        testthat::expect_true(
          question.correct,
          info = "Plot should use year on x, value on y, color = measure with ca_data, faceted by county_name, and log y-scale."
        )
